...
/*
        ...
        function:   prepare login cookies are created
                    ...
                    create token with type prepareLogin
...
class PrepareLoginCookie extends BaseClass {
    
    protected $debugOn = true;
    private $type = 'prepareLoginCookie';
    public function __construct( $appCode, $database, $user ){
        ...
        // debug
        $this->debug( 'Admin PrepareLoginCookie construct App code: ' . $appCode );        
    ...
    public function create( $request )
    {
        ...
        // debug
        $this->debug( 'admin PrepareLoginCookie create' );        
        ...
        $cookieToken = $token->create( $request,
                                       $this->type, 
                                       env( $this->appCode . '_ADMIN_AUTHORISATION_PREPARE_LOGIN_COOKIE_TOKEN_LENGTH' ), 
        ...
        if( $cookieToken ){
            ...
            // debug
            $this->debug( 'middleware admin PrepareLogin Cookie save ' );        
            $this->debug( 'token: ' . $cookieToken );        

            // get expiration period
            $expirationPeriod = env( $this->appCode . '_ADMIN_AUTHORISATION_PREPARE_LOGIN_EXPIRATION_PERIOD' );
    ...
    public function validate( $request ) {
        ...
        // debug
        $this->debug( 'middleware Admin PrepareLogin cookie validate' );        
